...
// Center + uniform scale to fit 2-unit bounding box, then sit on grid
function normalizeGeo(geo) {
  ...
  // Recompute after scale, then lift so bottom sits exactly on grid (y=0)
  geo.computeBoundingBox();
...
// ── Market data (sourced from WTO / Deloitte / McKinsey 2024 reports) ───────
const REGIONS = [
  { name:"East Asia",     share:36, growth:4.8, color:"#00d4ff" },
  { name:"North America", share:25, growth:3.2, color:"#ff8c35" },
  { name:"Europe",        share:21, growth:2.1, color:"#a855f7" },
  { name:"South Asia",    share:11, growth:8.1, color:"#22c55e" },
  { name:"Rest of World", share:7,  growth:3.5, color:"#f59e0b" },
];
const MAT_GROWTH = [
  { label:"Bioplastics",      pct:17.5 }, // ~17-18% CAGR per MarketsandMarkets 2024
  { label:"Carbon Fiber",     pct:11.2 }, // ~11% CAGR per Grand View Research 2024
  { label:"High-Strength Al", pct:5.8  }, // ~5-6% CAGR per Mordor Intelligence 2024
  { label:"Titanium Alloys",  pct:5.1  }, // ~5% CAGR per Fortune Business Insights 2024
  { label:"Stainless Steel",  pct:3.4  }, // ~3-4% CAGR per Allied Market Research 2024
];
...
export default function App() {
  const wrapRef     = useRef(null); // the .vwrap div — we measure this for Three.js sizing
  ...
  const matNameRef  = useRef("Steel"); // always-current material name, no stale closures
  ...
  useEffect(() => {
    ...
    // --- Camera (aspect corrected after first size) ---
    ...
    // --- Default placeholder mesh --- (lifted so bottom sits on grid y=0)
    ...
    // --- Size function — use wrap's clientWidth/Height ---
    const applySize = () => {
      const w = wrap.clientWidth, h = wrap.clientHeight;
      if (w > 0 && h > 0) {
        renderer.setSize(w, h, false); // false = don't set CSS (canvas CSS is 100%/100%)
        camera.aspect = w / h;
  ...
  // ── Update material on dropdown/swatch change ─────────────
  useEffect(() => {
  ...
  // ── Load parsed geometry into scene ──────────────────────
  const loadGeometry = useCallback((geo) => {
    ...
    // Normalize + create mesh with current material
    normalizeGeo(geo);
    ...
    if (camera) {
      const midY = r * 0.5; // model bottom at 0, top at ~2r, center at ~r
      camera.position.set(
    ...
    // Shadow catcher sits just below the grid (model bottom is at y=0)
      ...
      <div className="root">
        ...
        <div className="body">
          ...
          <div className="viewer">
            ...
            {/* .vwrap is the measured container; canvas is absolutely positioned inside */}
            <div className="vwrap" ref={wrapRef}
              onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
            >
              <canvas ref={canvasRef} />

              {/* Upload overlay — shown when no model loaded */}
              {!fileName && (
              ...
              {/* Reload button after model loaded */}
              {fileName && (
          ...
          <div className="panel">
            ...
            <div className="scroll">
              ...
              {activeTab==="market" && (
                <>
                  <div className="sec">Global Manufacturing</div>
                  <div className="card">
                    {[["Global Mfg Output (2024)","~$14.9T USD"],["YoY Growth (2024)","2.9%"],["Reshoring Activity","HIGH"],["Supply Chain Risk","ELEVATED"]].map(([l,v])=>(
                      <div className="mstat" key={l}><span className="mlbl">{l}</span><span className="mval">{v}</span></div>
                    ))}
                  </div>
                  <div className="sec">Regional Distribution</div>
                  <div className="card">
                    {REGIONS.map(r=>(
                  ...
                  <div className="sec">Technology Trends</div>
                  <div className="card">
                    {[["Additive Mfg Growth","19.3% CAGR"],["Smart Factory Adoption","~28% of sites"],
                      ["Industrial AI Spend","$47B by 2026"],["AI-Assisted Design","Rapid Adoption"],
                      ["Digital Twin Market","$73B by 2027"],["Net-Zero Target (2030)","43% of mfrs"]].map(([l,v])=>(
                      <div className="mstat" key={l}><span className="mlbl">{l}</span><span className="mval">{v}</span></div>
                    ))}
                  </div>
                  <div className="sec">Material Growth Rates</div>
                  <div className="card">
                    {MAT_GROWTH.map(m=>(
                  ...
                  <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:10,color:"#1a3a50",textAlign:"center",marginTop:10}}>
                    WTO · DELOITTE · GRANDVIEW RESEARCH — 2024 REFERENCE DATA
                  </div>
                </>
              )}
            </div>
